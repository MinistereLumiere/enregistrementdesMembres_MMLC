...
const REGULARITES = ["Actif", "Inactif", "En cours"];
...
function regulariteHTML(reg) {
  const cl = reg === "Actif" ? "pastille-actif"
    : reg === "Inactif" ? "pastille-inactif"
    : reg === "En cours" ? "pastille-encours"
    : "pastille-inconnu";
...
function renderFormGroupe(container, groupId) {
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <div style="position:relative;">
              <input type="text" id="responsable-recherche"
                     placeholder="Tapez pour rechercher un membre…"
                     value="${escapeHtml(g.responsableNom || "")}"
                     autocomplete="off">
              <input type="hidden" name="responsableId" id="responsable-id" value="${escapeHtml(g.responsableId || "")}">
              <div id="responsable-liste" style="display:none; position:absolute; top:100%; left:0; right:0; background:white; border:1px solid var(--gris-bordure); border-radius:8px; max-height:240px; overflow-y:auto; z-index:10; box-shadow:var(--ombre-douce);"></div>
            </div>
          </div>
        </div>
  ...
  // Filtrage du champ responsable
  const inputResp = $("#responsable-recherche");
  const listeResp = $("#responsable-liste");
  const hiddenResp = $("#responsable-id");

  function afficherListeResp(filtre = "") {
    const f = filtre.toLowerCase().trim();
    const candidats = state.membres
      .filter(m => !f || nomComplet(m).toLowerCase().includes(f))
      .slice(0, 50);
    listeResp.innerHTML = `
      <div class="resp-option" data-id="" style="padding:10px 14px; cursor:pointer; color:var(--gris-texte); border-bottom:1px solid #eee;">— Aucun —</div>
      ${candidats.map(m => `
        <div class="resp-option" data-id="${m.id}" data-nom="${escapeHtml(nomComplet(m))}" style="padding:10px 14px; cursor:pointer; border-bottom:1px solid #f5f5f5;">
          ${escapeHtml(nomComplet(m))} <span style="color:var(--gris-texte); font-size:12px;">— ${escapeHtml(m.role || "")}</span>
        </div>
      `).join("")}
      ${candidats.length === 0 ? `<div style="padding:14px; color:var(--gris-texte); text-align:center;">Aucun membre trouvé</div>` : ""}
    `;
    listeResp.style.display = "block";
    listeResp.querySelectorAll(".resp-option").forEach(opt => {
      opt.addEventListener("mousedown", (e) => {
        // mousedown avant blur pour que la sélection passe
        e.preventDefault();
        hiddenResp.value = opt.dataset.id || "";
        inputResp.value = opt.dataset.nom || "";
        listeResp.style.display = "none";
      });
    });
  }

  inputResp.addEventListener("focus", () => afficherListeResp(inputResp.value));
  inputResp.addEventListener("input", () => {
    // Si l'utilisateur retape, l'ancienne sélection ne tient plus
    hiddenResp.value = "";
    afficherListeResp(inputResp.value);
  });
  inputResp.addEventListener("blur", () => {
    setTimeout(() => { listeResp.style.display = "none"; }, 150);
  });
...
function calculerStats() {
  ...
  // Répartition de la régularité
  const parRegularite = [
    ["Actif", state.membres.filter(m => m.regularite === "Actif").length],
    ["Inactif", state.membres.filter(m => m.regularite === "Inactif").length],
    ["En cours", state.membres.filter(m => m.regularite === "En cours").length]
  ];

  // Données mensuelles de l'année sélectionnée (pour les comparaisons)
  const anneeRef = (state.filtreType === "ANNEE" || state.filtreType === "MOIS")
    ? state.filtreAnnee
    : new Date().getFullYear();

  const presencesParMois = Array(12).fill(0);
  const totalPresencesParMois = Array(12).fill(0);
  for (const p of state.presences) {
    const d = new Date(p.date);
    if (d.getFullYear() === anneeRef) {
      totalPresencesParMois[d.getMonth()]++;
      if (p.present) presencesParMois[d.getMonth()]++;
    }
  }
  // Taux de présence par mois (0-100) : présences / total enregistrements
  const tauxPresenceParMois = presencesParMois.map((nb, i) =>
    totalPresencesParMois[i] > 0 ? Math.round((nb / totalPresencesParMois[i]) * 100) : 0
  );

  // Inscriptions par mois pour l'année sélectionnée
  const inscritsParMois = Array(12).fill(0);
  for (const m of state.membres) {
    if (!m.dateAdhesion) continue;
    const d = new Date(m.dateAdhesion);
    if (d.getFullYear() === anneeRef) inscritsParMois[d.getMonth()]++;
  }
...
function renderStats(container) {
  ...
  container.innerHTML = `
    ...
    <h2 class="section-title">Répartition par régularité</h2>
    <div class="section-card">
      ${diagrammeRegularite(s.parRegularite)}
    </div>

    <h2 class="section-title">Comparaison mensuelle — Année ${s.anneeRef}</h2>
    <div class="section-card">
      <h3 style="font-size:14px; color:var(--gris-texte); margin-bottom:12px; text-transform:uppercase; letter-spacing:0.5px;">Taux de présence par mois (%)</h3>
      ${barresMensuelles(s.tauxPresenceParMois, true)}
    </div>
    <div class="section-card">
      <h3 style="font-size:14px; color:var(--gris-texte); margin-bottom:12px; text-transform:uppercase; letter-spacing:0.5px;">Nouveaux inscrits par mois</h3>
      ${barresMensuelles(s.inscritsParMois, false)}
    </div>
...
// Diagramme de la régularité avec couleur correspondant à la pastille
function diagrammeRegularite(donnees) {
  const total = donnees.reduce((s, d) => s + d[1], 0);
  if (total === 0) return `<p style="color:var(--gris-texte)">Aucun membre.</p>`;
  const couleurs = { "Actif": "#2E7D32", "Inactif": "#C62828", "En cours": "#9E9E9E" };
  // Barre empilée
  const segments = donnees
    .filter(([, v]) => v > 0)
    .map(([label, valeur]) => {
      const pct = (valeur / total) * 100;
      return `<div title="${escapeHtml(label)} : ${valeur}" style="width:${pct}%; background:${couleurs[label]};"></div>`;
    }).join("");
      ...
      ${donnees.map(([label, valeur]) => {
        ...
        return `
          <div style="display:flex; align-items:center; gap:8px;">
            <span style="display:inline-block; width:12px; height:12px; border-radius:50%; background:${couleurs[label]}"></span>
            <span><strong>${valeur}</strong> ${escapeHtml(label)} <span style="color:var(--gris-texte)">(${pct} %)</span></span>
          </div>`;
      }).join("")}
    </div>`;
}

// Barres verticales sur 12 mois (J F M A M J J A S O N D)
function barresMensuelles(valeurs, estPourcentage) {
  const mois = ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"];
  const max = Math.max(...valeurs, estPourcentage ? 100 : 1);
  const hauteurMax = 140;
  return `
    <div style="display:flex; align-items:flex-end; gap:6px; height:${hauteurMax + 30}px;">
      ${valeurs.map((v, i) => {
        const h = max > 0 ? (v / max) * hauteurMax : 0;
        return `
        <div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:flex-end; height:100%;">
          <div style="font-size:11px; color:var(--gris-texte); margin-bottom:4px;">${v}${estPourcentage && v > 0 ? "%" : ""}</div>
          <div title="${MOIS_LABELS[i]} : ${v}${estPourcentage ? " %" : ""}"
               style="width:100%; height:${h}px; background:${estPourcentage ? "var(--or)" : "var(--bleu-nuit)"}; border-radius:4px 4px 0 0; min-height:2px;"></div>
          <div style="font-size:11px; color:var(--gris-texte); margin-top:6px;">${mois[i]}</div>
        </div>`;
      }).join("")}
    </div>`;
}
